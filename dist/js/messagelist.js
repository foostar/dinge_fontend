	...
	(function ($) {
	    ...
	    MessageList.prototype = {
	        ...
	        getTemplate: function getTemplate(item) {
	            return "<div class='info_mini'>" + "<div class='info_slide'>" + "<div class='message_carouse'><a href='/views/message.html?mId=" + item.typeId + "'><img src='" + item.avatar + "'/></a></div>" + "<div class='message_list'>" + "<div class='message_nickname'><a href='/views/message.html?mId=" + item.typeId + "&name=" + encodeURI(encodeURI(item.username)) + "'>" + item.username + "</a></div>" + "<div class='message_date'>" + dingeTools.format(item.createdAt, "MM-dd") + "</div>" + "<div class='message_talk'>" + item.content + "</div>" + "</div>" + "</div>" + "<div class='del_info_btn' data-id='" + item.typeId + "'></div>" + "<div class='del_info_mask'></div>" + "</div>";
	        },
	        deleteFocus: function deleteFocus() {
	            // 删除slider
	            var self = this;
	        ...
	        loadMessageList: function loadMessageList(page) {
	        ...
	        showList: function showList() {
	            var self = this;
	            // 加载数据
	            self.loadMessageList(self.page).then(function (result) {
	                // 拼凑数据
	                ...
	                self.mySwiper = new Swiper(".swiper-container", {
	                    ...
	                    onTouchEnd: function onTouchEnd() {
	                        if (self.holdPosition > 100) {
	                            ...
	                            //加载新的slide
	                            self.page++;
	                            self.loadMessageList(self.page).done(function (result) {
	                                if (result.status == 1 && result.data.list.length > 0) {
	                                    var data = result.data.list;
	                                    data.forEach(function (item) {
	                                        var template = self.getTemplate(item);
